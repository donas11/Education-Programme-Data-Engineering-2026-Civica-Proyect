...
.detail{margin-top:1rem;background:var(--surface2);border:1px solid var(--divider);border-radius:1rem;padding:1.25rem}.section{margin-bottom:1.25rem}.section h3{font-size:.8rem;text-transform:uppercase;letter-spacing:.05em;color:var(--faint);border-bottom:1px solid var(--divider);padding-bottom:.55rem;margin-bottom:.8rem}.table{width:100%;border-collapse:collapse;font-size:.86rem}.table th,.table td{text-align:left;padding:.55rem;border-bottom:1px solid var(--divider);vertical-align:top}.code{white-space:pre-wrap;overflow:auto;max-height:340px;background:var(--bg);border:1px solid var(--border);border-radius:.8rem;padding:1rem;font-family:"JetBrains Mono",monospace;font-size:.8rem}.lineage{height:560px;border:1px solid var(--divider);border-radius:.85rem;background:var(--bg)}
.kv{display:grid;grid-template-columns:180px 1fr;gap:.5rem 1rem}.toolbar{display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.75rem}.toolbar select,.toolbar button{border:1px solid var(--border);background:var(--bg);color:var(--text);border-radius:.65rem;padding:.5rem .7rem}.empty{padding:2rem;background:var(--surface);border:1px solid var(--divider);border-radius:1rem;color:var(--muted)}
...
fetch('./site-data.json').then(r=>r.json()).then(data=>{
  ...
  let state={view:'all',selectedId:null,currentLayer:null,lineageMode:'both',lineageDepth:'all'};
  ...
  function collectLineage(rootId, mode, depthSetting){
    const maxDepth = depthSetting === 'all' ? Infinity : parseInt(depthSetting, 10);
    const graphNodes = new Map();
    const graphEdges = new Map();

    function addNode(id, role, level){
      const n = nodeMap[id];
      if(!n) return;
      const current = graphNodes.get(id);
      if(!current || Math.abs(level) > Math.abs(current.level||0)){
        graphNodes.set(id, {id, label:n.name, role, level, type:n.type});
      }
    }

    function addEdge(from, to){
      graphEdges.set(from+'->'+to, {from, to, arrows:'to'});
    }
    ...
    function walkUp(id, level){
      ...
      for(const parentId of (n.depends_on||[])){
        ...
        addNode(parentId, 'upstream', -level);
        addEdge(parentId, id);
        if(level < maxDepth) walkUp(parentId, level + 1);
      }
    }
    ...
    function walkDown(id, level){
      ...
      for(const childId of (childrenMap[id]||[])){
        ...
        addNode(childId, 'downstream', level);
        addEdge(id, childId);
        if(level < maxDepth) walkDown(childId, level + 1);
      }
    }

    addNode(rootId, 'selected', 0);
    if(mode === 'both' || mode === 'upstream') walkUp(rootId, 1);
    if(mode === 'both' || mode === 'downstream') walkDown(rootId, 1);
  ...
  function renderDetail(){
    ...
    document.getElementById('detail').innerHTML=`
      <div class="detail">
        ...
        <div class="section">
          <h3>Lineaje completo</h3>
          <div class="toolbar">
            <select onchange="window.__setLineageMode(this.value)">
              <option value="both" ${state.lineageMode==='both'?'selected':''}>Upstream + Downstream</option>
              <option value="upstream" ${state.lineageMode==='upstream'?'selected':''}>Solo upstream</option>
              <option value="downstream" ${state.lineageMode==='downstream'?'selected':''}>Solo downstream</option>
            </select>
            <select onchange="window.__setLineageDepth(this.value)">
              <option value="1" ${state.lineageDepth==='1'?'selected':''}>Profundidad 1</option>
              <option value="2" ${state.lineageDepth==='2'?'selected':''}>Profundidad 2</option>
              <option value="3" ${state.lineageDepth==='3'?'selected':''}>Profundidad 3</option>
              <option value="all" ${state.lineageDepth==='all'?'selected':''}>Completo</option>
            </select>
          </div>
        ...
        ${n.depends_on.length?`<div class="section"><h3>Dependencias</h3><div class="chips">${n.depends_on.map(d=>`<button class="chip-btn mono" onclick="window.__jumpTo('${d}')">${esc((nodeMap[d]?.name)||d.split('.').pop())}</button>`).join('')}</div></div>`:''}
        ${n.compiled_code?`<div class="section"><h3>SQL compilado</h3><div class="code">${esc(n.compiled_code)}</div></div>`:''}
      </div>`;
    renderLineage(n.id);
  }

  function renderLineage(rootId){
    const el=document.getElementById('lineage'); if(!el||typeof vis==='undefined') return;
    const graph = collectLineage(rootId, state.lineageMode, state.lineageDepth);
    const nodes = graph.nodes.map(n=>({
      ...
      shape:'box',
      margin:10,
      color:n.role==='selected'
        ? {background:'#4f98a3',border:'#4f98a3'}
        : n.role==='upstream'
          ? {background:'#5591c7',border:'#5591c7'}
    ...
    const network = new vis.Network(el,{nodes:new vis.DataSet(nodes),edges:new vis.DataSet(edges)},{
      layout:{hierarchical:{enabled:true,direction:'LR',sortMethod:'directed',levelSeparation:180,nodeSpacing:170,treeSpacing:220}},
      physics:false,
      interaction:{hover:true,navigationButtons:true,keyboard:true},
      nodes:{font:{face:'Inter',color:getComputedStyle(document.documentElement).getPropertyValue('--text').trim()}},
      edges:{smooth:false,arrows:{to:{enabled:true,scaleFactor:.7}}}
    });
  ...
  window.__setLineageMode=v=>{state.lineageMode=v; if(state.selectedId) renderDetail();};
  window.__setLineageDepth=v=>{state.lineageDepth=v; if(state.selectedId) renderDetail();};
